...
module.exports.getVisitorById = function (req, res) {
    ...
    VisitorModel.findById({ "_id": visitorId }).populate("visitorCategory").exec(function (err, data) {
        if (err) {
            res.json({ msg: "Something Went Wrong", status: -1, data: err })
        }
        else {
...
module.exports.getVisitorByHouse = function (req, res) {
    ...
    VisitorModel.find({ house: param_house }).populate("visitorCategory").exec(function (err, data) {
        if (err) {
            res.json({ msg: "Something Went Wrong", status: -1, data: err })
        }
        else {
